...
Immediately before finalizing the result, re-check the top option's supplier link one more time.

Confirm whether the price and MOQ still matches what was found earlier in the run.

- If it matches, note "confirmed at [time]" next to the top result.
- If it has changed or the option is no longer available, note "changed since first seen - verify before acting" and update the figure if a new one is visible.
