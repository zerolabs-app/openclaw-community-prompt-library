...
Job postings can close, fill, or be removed at any time, and listed salary ranges can be outdated.

The one-line Summary field at the top of the result file (used as the email subject/preview line) must not state a job as currently open as a guarantee.

The Summary field must include:

- "as of" framing with the time the posting was observed (from the run time already recorded)
- the top match's title, company, and source, so the user can verify it's still open in one tap

Example of a correct Summary line:

Summary: Best lead as of 2026-08-05 15:59 - DevOps Engineer at Acme Cloud (LinkedIn) - verify listing is still open before applying.

Example of an incorrect Summary line:

Summary: Found an open DevOps Engineer role at Acme Cloud.
...
Immediately before finalizing the result, re-check the top match's job posting link one more time.

Confirm whether the listing is still open still matches what was found earlier in the run.

- If it matches, note "confirmed at [time]" next to the top result.
- If it has changed or the posting is no longer available, note "changed since first seen - verify before acting" and update the figure if a new one is visible.
